"""Module to manage call offs dataframe"""
...
class CallOff():
    ...
    def get_data_frame(self):
        ...
        except FileNotFoundError ("File calloff_template.xlsx does not exist. Provide it to run "
                                  "program") as file_error:
            raise FileNotFoundError from file_error

    def give_single_order(self, row_index):
        """Gives complete row from dataframe as Series object, order number and infromation if
        order will be fully taken\
        or partially
