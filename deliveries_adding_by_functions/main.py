...
def create_working_dataframe(orders_dataframe, current_order_index, current_order):
	...
	working_orders_dataframe["Delivered_(kg)"] = working_orders_dataframe["Delivered_(kg)"].astype("int32")
...
def process_row_to_be_delivered(working_orders_dataframe, current_call_off_order):
	...
	working_orders_dataframe.loc[0, "Current_delivery_date"] = current_call_off_order["Confirmed_delivery_date"]
	working_orders_dataframe.loc[0, "Status"] = "confirmed"
	working_orders_dataframe.loc[0, "Required_delivery_date_OTIF1"] = current_call_off_order["Requested_date"]
	working_orders_dataframe.loc[0, "Required_quantity_OTIF1"] = current_call_off_order["Called_quantity_[kg]"]
	working_orders_dataframe.loc[0, "Ordered_(kg)"] = current_call_off_order["Confirmed_quantity_[kg]"]
	working_orders_dataframe.loc[0, "Delivered_(kg)"] = int(current_call_off_order["Confirmed_quantity_[kg]"])
...
def process_row_with_remaining_quantity(working_orders_dataframe):
	...
	working_orders_dataframe.loc[1, "Ordered_(kg)"] -= working_orders_dataframe.loc[0, "Ordered_(kg)"]
	working_orders_dataframe.loc[1, "Delivered_(kg)"] = \
		working_orders_dataframe.loc[1, "Delivered_(kg)"] - working_orders_dataframe.loc[0, "Delivered_(kg)"]
...
def change_order_part_postfix(working_orders_dataframe):
	...
	if "/" in working_orders_dataframe.loc[1, "Purchase_order_number"]:
		...
		working_orders_dataframe.loc[1, "Purchase_order_number"] = labour_list[0] + "/" + labour_list[1]
	else:
...
def assign_row_indexes(working_orders_dataframe, current_order_index, confirmed_second_part_row_index):
...
def main_loop(call_off_dataframe, orders_dataframe):
	...
	for index in range(len(call_off_dataframe["Customer_PO_number"])):
		...
		current_call_off_order, current_call_off_order_no = give_single_order(index, call_off_dataframe)
		current_order, current_order_index = match_order_from_call_off(orders_dataframe, current_call_off_order_no)
		working_orders_dataframe, orders_dataframe = create_working_dataframe(orders_dataframe, current_order_index, current_order)
		working_orders_dataframe = process_row_to_be_delivered(working_orders_dataframe, current_call_off_order)
		...
		confirmed_second_part_row_index = calculate_confirmed_part_row_index(working_orders_dataframe, orders_dataframe)
		working_orders_dataframe = assign_row_indexes(working_orders_dataframe, current_order_index, confirmed_second_part_row_index)
		orders_dataframe = add_working_rows_to_orders_dataframe(orders_dataframe, working_orders_dataframe)

	return orders_dataframe


orders_dataframe = give_complete_orders_dataframe(r"orders.xlsx")
call_off_dataframe = give_complete_call_off_dataframe(r"calloff_template.xlsx")
orders_dataframe = main_loop(call_off_dataframe, orders_dataframe)
write_dataframe_to_excel(orders_dataframe, "orders_output.xlsx")
